...
class LCD:
    ...
    def data_update(self, data):
        ...
        if self.adjusting_max:
            ...
            if data.minimum_cruise_ratio != self.printer.minimum_cruise_ratio:
                self.write("speed_settings.minimum_cruise_ratio.val=%d" % data.minimum_cruise_ratio)
    ...
    def _TempScreen(self, data):
        if data[0] == 0x01: # Hotend
            ...
            self.write("speed_settings.minimum_cruise_ratio.val=%d" % self.printer.minimum_cruise_ratio)
            ...
            unit = self.cruise_ratio/10
            if data[0] == 0x12:
                unit = -self.cruise_ratio/10
            cruise_ratio = self.printer.minimum_cruise_ratio + unit
            self.write("speed_settings.minimum_cruise_ratio.val=%d" % cruise_ratio)
            print(cruise_ratio)
            self.callback(self.evt.ACCEL_TO_DECEL, cruise_ratio)
            print(cruise_ratio)
            self.printer.minimum_cruise_ratio = cruise_ratio
            print(float(cruise_ratio))
